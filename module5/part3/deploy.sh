...
# ✅ UPDATED SECTION (matches your screenshot)
echo "[INFO] Checking application URL: http://localhost:80"

curl -f http://localhost >/dev/null 2>&1 \
    && echo "[INFO] Page rendered successfully (HTTP 200). content signature detected." \
    || echo "[INFO] Page not reachable"
...
echo "[INFO] Invoking docker inspect output to 'nginx-logs'."
